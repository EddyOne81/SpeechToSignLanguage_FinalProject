...
import type { FeedbackItem, FeedbackSortType } from "../types";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function FeedbackTab({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden glass-panel rounded-2xl p-5 shadow-lg">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">Your Feedbacks</h3>
        <button
          onClick={() => {
            setFeedbackSearch("");
      ...
      <p className="mt-1 text-xs text-slate-500">
        Submit feedback from any item in the History tab.
      </p>
      ...
      {feedbackError && (
        <div className="ui-alert-error mt-2 rounded-lg px-3 py-2 text-xs">
          {feedbackError}
        </div>
      )}
      <div className="mt-4 flex-1 min-h-0 space-y-3 overflow-y-auto">
        {!isLoggedIn ? (
          <p className="text-sm text-slate-500">Login to view your feedbacks.</p>
        ) : feedbackLoading ? (
          ...
          feedbackItems.map((item) => (
            <div key={item.feedbackId} className="glass-inset rounded-xl p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {item.historyId && (
                    <button
                      onClick={() => void openHistoryFromFeedback(item.historyId)}
                  ...
                  {item.historyId && (
                    <button
                      onClick={() =>
                        openFeedbackModal({
                          historyId: item.historyId as number,
                          historyText: item.historyText ?? undefined,
                          rating: item.rating ?? 5,
      ...
      {isLoggedIn && !feedbackError && (
        <div className="ui-divider-top mt-4 shrink-0 flex flex-wrap items-center justify-between gap-3 pt-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => void loadFeedbacks(Math.max(0, feedbackPage - 1))}
            ...
            {feedbackTotalPages > 1 && feedbackPage < feedbackTotalPages - 1 && (
              <button
                onClick={() => void loadFeedbacks(Math.max(feedbackTotalPages - 1, 0))}
                disabled={feedbackLoading}
                className="ui-btn-secondary h-9 flex items-center rounded-md px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50">
                Last
              </button>
